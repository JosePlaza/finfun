...
- Perfil y avatar (`src/ui/Profile.tsx`, `src/scene/avatars/`): botón bajo el patrimonio; el panel muestra el avatar en 3D, el nombre del jugador y su nivel, y (plegada) una colección de 30 avatares genéricos, cada uno con su cabeza dibujada como imagen fija por `src/ui/AvatarHead.tsx` (un lienzo oculto las genera al arrancar; nada de emojis), (pirata, bruja, robot, cactus, tiburón…) diseñados con el mismo kit low-poly. El elegido se guarda en el perfil de la cuenta (migración `0003_avatar.sql`) y pasea por la isla como primer vecino. Los vecinos y la Liebre (`NEIGHBORS`, `LIEBRE_AVATAR` en el catálogo) usan la misma morfología, así que toda la isla es un solo mundo.
- Cámara: girar con un dedo (o botón izquierdo), zoom con pinza (o rueda) y desplazarse por la isla con dos dedos (o botón derecho); el punto que mira la cámara no puede salirse de la isla y, si se aleja del centro, aparece el botón "Centrar isla".
- Los avisos del Mercado (dividendos, tormentas, noticias) llevan a "Mi cartera", donde los movimientos del mes salen marcados como nuevos, y desaparecen de Eventos al abrir el Mercado ese mes (`seenMarketMonth`).
- Insignias (`src/sim/badges.ts`, sección del Perfil): 14 familias con bronce/plata/oro (bellotas 100/500/1000, días jugados 7/30/100, dinero en el banco, lecciones, bonos, negocios, Fondo Isla, patrimonio, niveles, tormentas, visitas y préstamos a la Liebre, declaraciones, huerto). Las de un solo umbral (huerto ampliado) se tienen o no. Las conseguidas se apuntan en `badges` de la partida (no se pierden si el número baja) y salta un toast al ganarlas; `daysPlayed` cuenta los días con el juego abierto.
